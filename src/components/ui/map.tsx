...
type MapControlsProps = {
  ...
  showLocate?: boolean;
  /** Show 3D toggle button to tilt the map (default: false) */
  show3D?: boolean;
      ...
      {show3D && (
        <ControlGroup>
          <ControlButton
            ...
            label={is3D ? "Exit 3D mode" : "Enter 3D mode"}
          >
            <Box className={cn("size-4 transition-transform", is3D && "rotate-45 scale-110")} />
          </ControlButton>
        </ControlGroup>
      )}
      {showFullscreen && (
...
type MapPolygonProps = {
  ...
  id?: string;
  /** Array of [longitude, latitude] coordinate pairs defining the polygon ring. The first and last coordinates should be the same to close the ring. */
  coordinates: [number, number][];
  ...
  interactive?: boolean;
  /** Optional dash pattern [dash length, gap length] for the stroke line */
...
function MapPolygon({
  ...
  useEffect(() => {
    ...
    if (map.getLayer(strokeLayerId)) {
      ...
      if (strokeDashArray) {
        map.setPaintProperty(strokeLayerId, "line-dasharray", strokeDashArray);
      }
    }
  }, [isLoaded, map, fillLayerId, strokeLayerId, fillColor, fillOpacity, strokeColor, strokeWidth, strokeDashArray]);
  ...
  }, [isLoaded, map, fillLayerId, onClick, onMouseEnter, onMouseLeave, interactive]);
