...
import { Star, X, MapPin, Footprints, Locate, ShoppingBag, ExternalLink, Home as HomeIcon, Sparkles, Loader2, Coffee } from 'lucide-react';
                  ...
                  <div className='flex items-center gap-0.5'>
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`size-2.5 ${
                          i < Math.floor(selectedSight.tripadvisorRating ?? 0)
            ...
            <div className='mb-6 p-4 rounded-2xl bg-slate-50 border border-slate-100/50 group/fact relative overflow-hidden'>
              <div className='flex items-center justify-between mb-2'>
                ...
                {!isGenerating && (
                  <button
                    onClick={() =>
                      handleGenerateFact(selectedSight.name, selectedSight.id)
                    }
                    className='text-[10px] font-bold text-blue-600 hover:text-blue-700 transition-colors flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded-full hover:bg-blue-100 transition-all active:scale-95'
                  >
                    {funFacts[selectedSight.id] ? (
                      <>
                        Ancora!{' '}
                        <Coffee className='size-3 fill-blue-600/20' />
                      </>
                    ) : (
                      <>Узнать факт</>
                    )}
                  ...
                  <div className='flex flex-col'>
                    ...
                    {gpsLocation && (
                      <span className='text-[10px] font-bold text-white/70'>
                        {formatDistance(
                          haversineDistance(gpsLocation, selectedShop.coords),
                        )}{' '}
                        away
                      </span>
                    )}
        ...
        <Card className='pointer-events-auto w-full max-w-[24rem] mx-auto rounded-[2rem] overflow-hidden shadow-2xl bg-white border-none ring-1 ring-black/5 animate-in slide-in-from-bottom-8 duration-300'>
          <div className='p-6 flex flex-col'>
            ...
            <div className='flex gap-2.5'>
              ...
              >
                <Footprints className='size-3.5 mr-1.5 text-rose-400' /> Walk To
                Home
              </Button>
            </div>
          </div>
